...
import {deploy_complete, deploy_failed, deploy_inProgress, templateCheck_failed, templateCheck_inProgress, provision_failed, provision_inProgress} from "../fixtures/deployment-state.mock";
...
describe("Deployment", () => {
  ...
  context.only("Deployment state", () => {
    ...
    specify("Template check IN_PROGRESS", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, templateCheck_inProgress) // 1

      // 1
      cy.get("[data-checkpoint='Template check']").should("have.attr", "data-status", STATUS.IN_PROGRESS)
      cy.get("[data-checkpoint='Provision']").should("have.attr", "data-status", STATUS.PENDING)
      cy.get("[data-checkpoint='Deploy']").should("have.attr", "data-status", STATUS.PENDING)
    })

    specify("Provision IN_PROGRESS", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, provision_inProgress) // 2

      cy.get("[data-checkpoint='Template check']").should("have.attr", "data-status", STATUS.COMPLETED)
      cy.get("[data-checkpoint='Provision']").should("have.attr", "data-status", STATUS.IN_PROGRESS)
      cy.get("[data-checkpoint='Deploy']").should("have.attr", "data-status", STATUS.PENDING)
    })

    specify("Deploy IN_PROGRESS", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, deploy_inProgress) // 3

      cy.get("[data-checkpoint='Template check']").should("have.attr", "data-status", STATUS.COMPLETED)
      cy.get("[data-checkpoint='Provision']").should("have.attr", "data-status", STATUS.COMPLETED)
      cy.get("[data-checkpoint='Deploy']").should("have.attr", "data-status", STATUS.IN_PROGRESS)
    })

    specify("Template check FAILED", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, templateCheck_failed) // 4

      cy.get("[data-checkpoint='Template check']").should("have.attr", "data-status", STATUS.FAILED)
      cy.get("[data-checkpoint='Provision']").should("have.attr", "data-status", STATUS.PENDING)
      cy.get("[data-checkpoint='Deploy']").should("have.attr", "data-status", STATUS.PENDING)
    })

    specify("Provision FAILED", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, provision_failed) // 5

      cy.get("[data-checkpoint='Template check']").should("have.attr", "data-status", STATUS.COMPLETED)
      cy.get("[data-checkpoint='Provision']").should("have.attr", "data-status", STATUS.FAILED)
      cy.get("[data-checkpoint='Deploy']").should("have.attr", "data-status", STATUS.PENDING)
    })

    specify("Deploy FAILED", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, deploy_failed) // 6

      cy.get("[data-checkpoint='Template check']").should("have.attr", "data-status", STATUS.COMPLETED)
      cy.get("[data-checkpoint='Provision']").should("have.attr", "data-status", STATUS.COMPLETED)
      cy.get("[data-checkpoint='Deploy']").should("have.attr", "data-status", STATUS.FAILED)
    })

    specify("Deploy COMPLETED", () => {
      cy.intercept(`users/${user1.id}/applications/${app1.id}/deployments/${depl1.id}/states`, deploy_complete) // 7
