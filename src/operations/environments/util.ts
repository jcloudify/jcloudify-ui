import {DatabaseConf1WithDatabaseEnum, OneOfPojaConf, PojaConf1} from "@jcloudify-api/typescript-client";
...
const normalizeGenApiClientConf = ({__flags, gen_api_client}: PojaConfFormDataV1) => {
  if (!__flags?.with_gen_clients) return undefined;
  const {codeartifact_domain_name, codeartifact_repository_name, aws_account_id, ...rest} = gen_api_client!;

  const publishConfig = gen_api_client?.with_publish_to_npm_registry ? {
    codeartifact_domain_name,
    codeartifact_repository_name,
    aws_account_id,
  } : {codeartifact_repository_name: undefined, codeartifact_domain_name: undefined, aws_account_id: undefined};
